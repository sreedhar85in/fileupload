...
@EnableSwagger2
@Configuration
@ComponentScan({"com.Javatips.framework.fileupload.controller"})
public class SwaggerConfig implements WebMvcConfigurer {
	...
	public Docket userApi() {
		...
		return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()).select().apis(RequestHandlerSelectors.basePackage("com.Javatips.framework.fileupload.controller"))
				.build()
...
private ApiInfo apiInfo() {
		...
		return new ApiInfoBuilder().title("API for uploading a file").description("This project is for implementing the file uploading").version("1.0.0").license("Apache 2.0").licenseUrl("http://www.apache.org/licenses/LICENSE-2.0")
				.build();
...
private SecurityContext securityContext() {
	return SecurityContext.builder().securityReferences(defaultAuth()).forPaths(PathSelectors.regex("/uploadapi.*"))
			.build();
}
